...
\textcolor{red}{The particular form of $J$ means that we get normal continuity of the velocity in the horizontal direction with any $H(\text{div})$-conforming element like BDFM, so we can use discontinuous pressure.
If we take a tensor product with the right thing in the vertical and do something sensible for the vertical velocity then we get full $H(\text{div})$-conformity in the whole space.
Work this out and explain.}
...
Implementing this discretization is notoriously error-prone \citep{houston2018automatic}.
...
Our end goal is to solve the Stokes equations using DG methods because the thickness is also discretized.
But we can test the ideal form of the Stokes equations in an alternative coordinate system by first using exact expressions for the bed and surface topography.
This test lets us check that we had the correct reformulation of the Stokes equations first, and then check our DG implementation after.
